...
def plot_results(exp: Experiment, config: dict) -> Figure:
    """
    ...
    ax.plot(x_line, y_line, "b--", linewidth=2, alpha=0.7, label="Calibration Fit")
    ...
    for group, samples in config["groups"].items():
        ...
        ax.axhline(y_samples.mean(), c=pc.get_facecolor())
